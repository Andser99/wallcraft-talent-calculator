...
export function addTalentsForTabId(tree: TalentData, tabId: number, treeName: string, icons: Record<string, string>){
     for (let talent of talentJson.filter(_ => _.TabID === tabId)){
        ...
        let newTalent: Talent = {
          ...
          description: (num: number) => parseSpellValues(`${spells[num] !== undefined ? spells[num-1]["Description_enUS"]: spells[0]["Description_enUS"]}`, spells[num-1]),
          icon: icons[iconDictionary[spells[0]["SpellIconID"]]]};
